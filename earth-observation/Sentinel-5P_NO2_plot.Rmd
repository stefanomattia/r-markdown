...
PlotRegion <- function(df, latlon, title) {
  ...
  df_sub <- subset(df, lat>latlon[1] & lat<latlon[2] & lon>latlon[3] & lon<latlon[4])

  ggplot(df_sub, aes(y=lat, x=lon, fill=no2tc)) + 
    geom_tile(width=1, height=1) +
    borders('world', xlim=range(df_sub$lon), ylim=range(df_sub$lat), colour='gray80', size=.2) + 
    theme_light() + 
    theme(panel.ontop=TRUE, panel.background=element_blank(), plot.title = element_text(hjust=0.5)) +
    scale_color_distiller(palette='Spectral') +
    scale_fill_distiller(palette='Spectral') +
    coord_quickmap(xlim=c(latlon[3], latlon[4]), ylim=c(latlon[1], latlon[2])) +
    labs(title=title, x="Longitude", y="Latitude", fill=expression(mol~m^-2))
}
```

There is a lot going on when calling the `ggplot` function, and the syntax might look a bit puzzling at first. Basically, we first need to define the base plot specifying the data to use and the aesthetics, and after that we can add more layers and themes to it.
...
It doesn't look much better, especially if you live on the east coast!
...
*This post was written entirely in R markdown. You can download it on my [GitHub repository](https://github.com/stefanomattia/r_markdown/tree/master/earth-observation).*  
